...
public class AuthController {
    ...
    public @ResponseBody
    FindCustomerResponse getCustomer(@RequestBody FindCustomerRequest customerRequest) {
    ...
    @PostMapping(value = "/get-disputeInformation", consumes = "application/json", produces = "application/json")
    public @ResponseBody
    DisputeResponse getCustomer(@RequestBody Customer customer,  @RequestHeader(name = "Content-Type") String Content) {
        {
            ...
            var customerDetails = new DisputeResponse();
            customerDetails.setCustomerId(customer.getCustomerId());

            HttpEntity<DisputeResponse> request = new HttpEntity<>(customerDetails, headers);


            ResponseEntity<DisputeResponse> deliveryRequestResponse = restTemplate.postForEntity("http://localhost:8087/get-dispute-by-customer", request, DisputeResponse.class);


            DisputeResponse dbDisputeResponse = new DisputeResponse();
            dbDisputeResponse.setDisputeId(deliveryRequestResponse.getBody().getDisputeId());


            return dbDisputeResponse;
